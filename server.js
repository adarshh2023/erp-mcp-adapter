...
function toolCatalog() {
  ...
  // duplicate inputSchema under snake_case for each
  tools.forEach((t) => {
    if (!t.input_schema) t.input_schema = t.inputSchema;
  });
  return tools;
}

// ---------- MCP JSON-RPC handlers (root, /tools/list, /tools/call, /mcp)

// health
app.get("/", (_req, res) => res.json({ ok: true, service: "erp-mcp-adapter" }));
app.options("/", (_req, res) => res.sendStatus(204));

// root JSON-RPC multiplexer (Agent Builder may POST to "/")
app.post("/", async (req, res) => {
  ...
  try {
    ...
    return res.status(err?.status || 500).json({
      jsonrpc: "2.0",
      id,
      result: { content: [{ type: "json", json: err }], isError: true },
    });
  }
});
